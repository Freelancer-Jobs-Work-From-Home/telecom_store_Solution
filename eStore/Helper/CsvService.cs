...
namespace eStore.Helper
{
    public class CsvService
    {
        ...
        /// </summary>
        public async Task<(List<ProductViewModel>, string)> PreviewCsvAsync(IFormFile file)
        {
            ...
            using (var csv = new CsvReader(stream, new CsvConfiguration(CultureInfo.InvariantCulture)))
            {
                try
                {
                    previewData = csv.GetRecords<ProductViewModel>().ToList();
                    ...
                    foreach (var item in previewData)
                    {
                        if (!allCategories.TryGetValue(item.CategoryName, out var categoryId))
                            return (null, $"Danh mục '{item.CategoryName}' không tồn tại!");
